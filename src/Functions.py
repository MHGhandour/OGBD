...
def initialize_cavity(MainPts,level,start_index,*slicE):
    index_in=np.zeros_like(MainPts[:,0],dtype=bool)
    index_in[start_index]=True;
...
def enrichment(MainPts,Gi_splash,level1,level2,*slicE):
    ...
    return(Gi_splash)
#%%Progrossive
def Progressive(MainPts,Gi_splash,*slicE):
    for level in range(min(MainPts[:,3]).astype(int),max(MainPts[:,3]).astype(int)):
        Gi_splash=enrichment(MainPts,Gi_splash,level,level+1,slicE[0])    
    return(Gi_splash)
#%% Regressive
def Regressive(MainPts,Gi_splash,*slicE):
    ...
    return(Gi_splash)
#%% LAPS
def LAPS(MainPts,Gi_splash,Gi_splash2,*slicE):
    #print(' New Lap')
    Gi_splash=Regressive(MainPts,Gi_splash,slicE[0])
    Gi_splash3=np.zeros_like(Gi_splash);Gi_splash3[:]=Gi_splash[:]
    if(~np.sum(Gi_splash2^Gi_splash3,dtype=bool)):
        return(Gi_splash)
    Gi_splash=Progressive(MainPts,Gi_splash,slicE[0])
    Gi_splash2=np.zeros_like(Gi_splash);Gi_splash2[:]=Gi_splash[:]
    if(~np.sum(Gi_splash2^Gi_splash3,dtype=bool)):
        return(Gi_splash)
    Gi_splash=LAPS(MainPts,Gi_splash,Gi_splash2,slicE[0])
